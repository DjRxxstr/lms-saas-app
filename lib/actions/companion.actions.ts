...
export const createCompanion
    = async (formData: CreateCompanion) => {
        ...
        if (error || !data) throw new Error(
            error?.message || 'Failed to create a companion'
        )
...
export const newCompanionPermission = async () => {
    ...
    // Should be the names given as slug in clerk
...
// export const addToBookmarks = async (id: string) => {
...
//         throw new Error(fetchError?.message || 'Companion not found');
//     }
...
//         throw new Error(insertError?.message || 'Failed to add bookmark');
//     }
...
//     let query = supabase.from('bookmarked_companions').select();
